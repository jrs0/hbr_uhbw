...
Planned major non-deferrable non-cardiac surgery on DAPT after PCI is a major ARC criterion:
...
NOTE: long-term use (similar to OAC) is not determined by the timescale, it is obtained from the indication + the prescriptions (i.e. if the patient has a condition that would imply they are on long-term NSAID).
