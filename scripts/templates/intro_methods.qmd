...
\begin{itemize}
    \item The primary diagnosis of the first episode must contain an ACS ICD-10 code; or
    \item There must be a PCI procedure OPCS-4 code in any primary or secondary position in the first episode of the spell.
\end{itemize}
...
We randomly split the data into a training set ({{ 1 - test_set_proportion}}) and a testing set ({{ test_set_proportion }}). The testing set is not used in the model fitting process. We trained multiple models on each training set:
