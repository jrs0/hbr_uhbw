...
num_invasive = num_over_70 - num_conservative
```

The total number of index ACS events is `{python} num_index` between the dates `{python} index_start` and `{python} index_end`. Of these, `{python} num_over_70` are patients over 70. Of these, `{python} num_invasive` are managed invasively (PCI/CABG), and `{python} num_conservative` are managed conservatively.
...
# Restrict to invasive group
invasive = info_management["group"] != "Conservative"
...
ax.step(time, survival_prob, where="post")
ax.fill_between(
    time, conf_int[0], conf_int[1], alpha=0.25, step="post", label="Invasive"
)
